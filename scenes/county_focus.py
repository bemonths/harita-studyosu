...
Başlık ve renk açıklaması yoktur. Açılış kadrajında eyalet ekranın ortasındadır (üstte ve altta en az %8 boşluk);
yakınlaşma bu kadrajdan başlar. Etiket county'ye binmez, mümkünse su ya da eyalet dışı üstüne konur.
...
def setup(ctx):
    p, fig = ctx.p, ctx.fig
    g = maplib.MapGeometry(p, label_side="auto", region=framing.CENTER_REGION)
    ...
    def update(t):
        # 0,0–0,4 sabit, ortalı eyalet kadrajı; 0,4–2,2 county'ye yakınlaşma
    ...
    update.geometry, update.layers = g, m
    return update
